...
def print_custom_help():
    ...
    print("\nMotion estimation search pattern (five candidates: the collocated block plus four neighbours):")
    print('--heuristic               Pattern shape: "diamond" (default, neighbours on the axes) or')
    print('                          "square" (neighbours on the diagonals).')
    print('--me-offset               Neighbour offset in pixels. Default 2.')
...
def _add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('-cf', '--colorfulness', action='store_true', default=False)
    parser.add_argument('-pm', '--plot_metrics', '-plot_metrics', type=int, nargs='?', const=1, default=0)
    parser.add_argument('-cc', '--chroma_complexity', action='store_true', default=False)
    parser.add_argument('-me', '--motion_estimation', action='store_true', default=False)
    ...
    # Choices are spelled out here, rather than imported from libs.temporal_engine, so
    # `--help` does not have to import torch; tests/test_temporal_engine.py keeps the
    # two in sync.
    parser.add_argument('--heuristic', type=str, default='diamond',
                        choices=['diamond', 'square'])
    parser.add_argument('--me-offset', dest='me_offset', type=int, default=2,
                        help='neighbour offset in pixels (>= 1)')

    # --- Motion compensation strategy ---
    parser.add_argument('--mc', dest='mc', type=str, default='dense_smooth',
                        choices=['dense_smooth', 'dense', 'block', 'obmc'])
    parser.add_argument('--mc-smooth', dest='mc_smooth', type=str, default='gauss',
                        choices=['gauss', 'median', 'none'])
    parser.add_argument('--residual-dc', dest='residual_dc', action='store_true', default=False)
...
def get_parser_arguments(argv=None) -> argparse.Namespace:
    ...
    if args.me_offset < 1:
        parser.error(f'--me-offset must be >= 1, got {args.me_offset}')
    return args
