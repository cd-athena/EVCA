...
def test_diamond_epe_on_axis_shifts(vy, vx):
    ...
    matcher = SparsePatternBlockMatcher(32, 'diamond', offset=OFFSET)
    mvs, _ = matcher(_to_batch(frames[2]), _to_batch(frames[1]))
    assert _epe(mvs, vy, vx) < 1e-6


@pytest.mark.parametrize('offset', [1, 3, 5])
def test_odd_shifts_are_exactly_representable(offset):
    """Searching at full resolution makes odd motion vectors exact.

    The previous half-resolution search could only ever emit even MVs, so an odd shift
    carried at least 1 px of endpoint error by construction.
    """
...
def test_square_epe_diagonal():
    ...
    matcher = SparsePatternBlockMatcher(32, 'square', offset=3)
    mvs, _ = matcher(_to_batch(frames[2]), _to_batch(frames[1]))
...
def test_diamond_cannot_reach_diagonal_motion():
    ...
    diamond = SparsePatternBlockMatcher(32, 'diamond', offset=2)(curr, ref)[0]
    square = SparsePatternBlockMatcher(32, 'square', offset=2)(curr, ref)[0]
